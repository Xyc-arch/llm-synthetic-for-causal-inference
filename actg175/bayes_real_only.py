...
OUT_JSON = ACTG_DIR / "results" / "bayesian_tmle_curve_real_only.json"
...
# Real-only subsampling curve.
REAL_SUBSAMPLE_SIZES = list(range(50, 601, 50))
REAL_REPS_PER_SIZE = 20

# Real-only alpha prior.
# No synthetic information: use a uniform prior over this range.
ALPHA_MIN = -2 / 3
ALPHA_MAX = -1 / 2
...
# Random seeds.
# RANDOM_SEED controls posterior sampling.
# SUBSAMPLE_SEED_BASE controls the real-data subsamples.
# This is aligned with the synthetic-assisted script.
RANDOM_SEED = 12345
SUBSAMPLE_SEED_BASE = 700000
...
def run_real_subsampling_curve(df: pd.DataFrame):
    ...
    This does not use synthetic data and does not use full-data TMLE as truth.

    The seed schedule is aligned with the synthetic-assisted script:
        seed = SUBSAMPLE_SEED_BASE + 1000 * m + r
    """
    ...
    for m in usable_sizes:
        ...
        print(f"Real-only subsampling: m={m}, reps={REAL_REPS_PER_SIZE}")
...
# =========================
# Bayesian real-only curve extrapolation
# =========================
...
def bayesian_curve_real_only(
    ...
    Full-data TMLE is not used as truth.
    """
    ...
    for alpha in alpha_grid:
        ...
        # Uniform prior over alpha, so log_alpha_prior is constant.
        log_alpha_prior = 0.0

        obs_cov = np.diag(se_values ** 2)

        # Marginal likelihood integrating theta:
    ...
    return {
        ...
        "alpha_prior": {
            ...
            "uses_synthetic_information": False,
        },
        "priors": {
...
def main():
    ...
    bayes = bayesian_curve_real_only(
    ...
    print(f"\nSaved real-only Bayesian curve extrapolation to: {OUT_JSON}")
