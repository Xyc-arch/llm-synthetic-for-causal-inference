...
def evaluate_dataset(path, estimator_fn, estimator_name, covariates, ate_true):
    ...
    df[TREATMENT_COL] = pd.to_numeric(df[TREATMENT_COL], errors="coerce").round().astype("Int64")
    df[OUTCOME_COL] = pd.to_numeric(df[OUTCOME_COL], errors="coerce").round().astype("Int64")
...
def evaluate_setting(setting_dir: Path):
    ...
    for dataset_name, path in candidate_datasets(setting_dir).items():
        ...
        for est_name, est_fn in estimators.items():
            print(f"Running setting={setting_dir.name} dataset={dataset_name} estimator={est_name}")
            ...
            try:
                ...
                print(f"ERROR setting={setting_dir.name} dataset={dataset_name} estimator={est_name}: {e}")
...
def main():
    ...
    if args.data_dir is not None:
        ...
        setting_dirs = [
            ...
            if p.is_dir() and (p / "data_seed.csv").exists() and (p / "truth.json").exists()
        ]
