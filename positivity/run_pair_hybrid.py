...
def make_pair_qhyb(orig_path, syn_cov_path, syn_hyb_path, out_csv, threshold=THRESHOLD):
    ...
    rf_q_hyb = fit_q_on_hybrid(syn_hyb)
    ...
    for _, row in orig_std[orig_std["ps_hat_raw"] > (1 - threshold)].iterrows():
        ...
        matched_rows.append(new_row.to_dict())
        used_idx.add(best_idx)
        matched_high_count += 1

    if len(matched_rows) > 0:
        matched_df = pd.DataFrame(matched_rows)
        y_prob, y_draw = predict_y_from_q(rf_q_hyb, matched_df)
        matched_df["y_prob_qhyb"] = y_prob
        ...
        matched_df = pd.DataFrame(columns=COVARIATES + [TREATMENT_COL, OUTCOME_COL, "y_prob_qhyb"])
    ...
    diagnostics = {
        ...
        "matched_low_added_as_treated": int(matched_low_count),
        "matched_high_added_as_control": int(matched_high_count),
